...
void USTUGameOverWidget::NativeOnInitialized()
{
	...
	if (GetWorld())
	{
		...
		if (GameMode)
		{
			GameMode->OnMatchStateChanged.AddUObject(this, &USTUGameOverWidget::OnMatchStatChanged);
		}
	}
...
void USTUGameOverWidget::OnMatchStatChanged(ESTUMatchState Stat)
{
	if (Stat == ESTUMatchState::GameOver)
	{
		UpdatePlayerStat();
	}
}
...
void USTUGameOverWidget::UpdatePlayerStat()
{
	...
	for (auto It = GetWorld()->GetControllerIterator(); It; ++It)
	{
		...
		PlayerStatRowWidget->SetTeamImage(PlayerState->GetTeamColor());
